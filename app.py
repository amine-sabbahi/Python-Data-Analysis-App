...
def creat_dataset():
    ...
    if os.path.exists(creat_file_path):
        ...
        CTkMessagebox(title="Warning",
                      message=f"Failed to creat dataset",
                      icon="cancel",
                      option_1="Ok")
        ...
        if target_name != "" and target_type != "":
            if target_name in columns_list:
                ...
                CTkMessagebox(title="Error",
                              message=f"There is no column named {target_name} in the imported dataset",
                              icon="cancel",
                              option_1="Ok")
        else:
            CTkMessagebox(title="Error",
                          message="Please specify the target column name and type first.",
                          icon="cancel",
                          option_1="Ok")
    else:
        CTkMessagebox(title="Error",
...
def delete_columns():
    ...
    if cnt == len(columns_to_drop):
        ...
        CTkMessagebox(title="Success",
                      message=f"Columns {" ".join(columns_to_drop)} deleted successfully.",
                      icon="check",
                      option_1="Ok")
        column_entry.delete(0, 'end')
    else:
        CTkMessagebox(title="Error",
                      message="Please enter columns name correctly.",
                      icon="cancel",
                      option_1="Ok")
        column_entry.delete(0, 'end')
def update_table():
...
def standardize_features():
    global dataset, sc_entry
    if dataset is not None:
        cols = sc_entry.get()
        columns_to_nor = cols.split(',')
        cnt = 0
        for col in columns_to_nor:
            if col in dataset.columns.tolist():
                cnt += 1
        if cnt == len(columns_to_nor):
            # Create a MinMaxScaler instance
            scaler = StandardScaler()
            # Normalize the numerical columns
            dataset[columns_to_nor] = scaler.fit_transform(dataset[columns_to_nor])
            update_table()
            CTkMessagebox(title="Success",
                          message=f"Features Standardize successfully.",
                          icon="check",
                          option_1="Ok")
        else:
            CTkMessagebox(title="Error",
...
def normalize_features():
    ...
    if dataset is not None:
        ...
        if cnt == len(columns_to_nor):
            ...
            update_table()
            CTkMessagebox(title="Success",
                          message=f"Features Normalized successfully.",
                          icon="check",
                          option_1="Ok")
        else:
            CTkMessagebox(title="Error",
...
def handle_data_by_method():
    ...
    if dataset is not None:
        ...
        if method != "" and columns_to_handle != "":
            ...
            # Convert the column to numeric (ignore errors for non-convertible values)
            ...
            update_info_list()
            CTkMessagebox(title="Success",
                          message=f"Missing Values Handled successfully.",
                          icon="check",
                          option_1="Ok")
        else:
            CTkMessagebox(title="Error",
                          message="Please specify method and features.",
                          icon="cancel",
                          option_1="Ok")
        mv_features.delete(0, 'end')
        check_nan()
...
def handle_data_by_value():
    ...
    if dataset is not None:
        ...
        if value != "" and features != "":
            ...
            CTkMessagebox(title="Error",
                          message="Please specify value and features correctly.",
                          icon="cancel",
                          option_1="Ok")
...
def data_encoding():
    ...
    if dataset is not None:
        if feature in dataset.columns.tolist():
            ...
            update_table()
            CTkMessagebox(title="Success",
                          message=f"The data encoded successfully.",
                          icon="check",
                          option_1="Ok")
        else:
            CTkMessagebox(title="Error",
                          message="Please enter a feature from the dataset.",
                          icon="cancel",
                          option_1="Ok")
    ...
    if dataset is not None:
        ...
        if file_path:
            try:
                ...
                CTkMessagebox(title="Success",
                              message=f"Dataset saved successfully to:\n{file_path}",
                              icon="check",
                              option_1="Ok")
    ...
    if model != "Choose an algorithm":
        ...
        if filename:
            if target_type == "Qualitative values":
                ...
                pdf.add_page()
                pdf.set_font('Arial', 'B', 10)
                pdf.cell(0, 10, 'Dataset after Preprocessing:', ln=True, align='C')
                pdf.ln(10)
                ## Show table of historical data
                output_df_to_pdf(pdf, dataset.head(40))
...
# head_label = ctk.CTkLabel(master=head_frame, text='Data Analysis Toool', font=('bold', 40), text_color='#e06d14')
# head_label.place(relx=0.5, rely=0.5, anchor='center')
...
def import_page():
    ...
    import_label = ctk.CTkLabel(import_frame, text="Import or Creat\n  DataSet:", font=('bold', 20))
    ...
    creat_btn = ctk.CTkButton(master=import_frame, font=("bold", 18), text="Click to Creat", fg_color="green",
...
def process_page():
    global dataset, dataset_info_var, process_frame, column_entry, nr_entry, sc_entry, check_label_var, process_frame, mv_features, method_option, value_entry, na_by_value_entry, encodin_value_entry, encodin_cat_value_entry, encoding_entry, check_dd_var
    if dataset is not None:
        ...
        tabview.add("Features Selection")
        ...
        # Dlete Features
        dl_columns = ctk.CTkFrame(tabview.tab("Features Selection"))
        ...
        col_label.place(relx=0.65, rely=0.08)

        column_entry = ctk.CTkEntry(master=dl_columns_left, placeholder_text="Enter list of columns separated with ','",
                                    width=250)
        column_entry.place(relx=0.65, rely=0.25)

        dl_btn = ctk.CTkButton(master=dl_columns_left, text="Delete", font=('bold', 12), width=250, command=delete_columns)
        dl_btn.place(relx=0.65, rely=0.5)

        save_dl_btn = ctk.CTkButton(master=dl_columns_left, text="Save Current Dataset", font=('bold', 13), width=250,
                                    fg_color='green', command=export_dataset)
        save_dl_btn.place(relx=0.65, rely=0.7)


        dataset_info_var = tk.StringVar()
        dataset_info_label = ctk.CTkLabel(dl_columns_left, width=150, height=190, font=('bold', 11),
                                          textvariable=dataset_info_var, justify='center')
        dataset_info_label.place(relx=0.27, rely=0.02)
        ...
        #drop_duplic = ctk.CTkFrame(tabview.tab("Features Selection"))
        #drop_duplic.pack(side='right', fill="both", expand=True)

        result_dd = ctk.CTkButton(master=dl_columns_right, text="Drop Duplicated Rows", width=250,
                                  command=delete_duplicated_data)
        ...
        option_label = ctk.CTkLabel(ms_values, text="Choose methode to hande each column", font=("bold", 12), padx=0,
                                    pady=0)
        option_label.place(relx=0.45, rely=0.5, anchor="center")

        method_option = ctk.CTkOptionMenu(ms_values,
                                          values=["Previous value", "Next value", "Delete row", "Mean", "Max", "Min"])
        ...
        export_data_btn = ctk.CTkButton(ms_values, text="Save Current Dataset", font=('bold', 13), width=250,
                                        fg_color='green',
                                        command=export_dataset)
        export_data_btn.place(relx=0.7, rely=0.92, anchor="center")

        # Features Scaling
        # Data Normalization
        nr_data = ctk.CTkFrame(tabview.tab("Feature Scaling"))
        nr_data.pack(side='left', fill="both", expand=True)

        nr_title = ctk.CTkLabel(nr_data, text="Data Normalization:", font=('bold', 17))
        nr_title.place(relx=0.5, rely=0.05, anchor='center')
        nr_sub = ctk.CTkLabel(nr_data,
                              text="Normalization scales the numerical values in the dataset to a range between 0 and 1.",
                              text_color="light grey")
        nr_sub.place(relx=0.5, rely=0.15, anchor='center')

        nr_label = ctk.CTkLabel(master=nr_data, text="Enter features to Normalize:", font=('bold', 15))
        nr_label.place(relx=0.5, rely=0.25, anchor='center')

        nr_entry = ctk.CTkEntry(master=nr_data, placeholder_text="Enter list of features separated with ','", width=250)
        nr_entry.place(relx=0.5, rely=0.4, anchor='center')

        nr_btn = ctk.CTkButton(master=nr_data, text="Normalize", font=('bold', 12), width=250,
                               command=normalize_features)
        nr_btn.place(relx=0.3, rely=0.5)

        save_nl_btn = ctk.CTkButton(master=nr_data, text="Save Current Dataset", font=('bold', 13), width=250,
                                    fg_color='green', command=export_dataset)
        save_nl_btn.place(relx=0.5, rely=0.8, anchor='center')

        # Data Standarlization
        sc_data = ctk.CTkFrame(tabview.tab("Feature Scaling"))
        sc_data.pack(side='right', fill="both", expand=True)

        sc_title = ctk.CTkLabel(sc_data, text="Data Standardization:", font=('bold', 17))
        sc_title.place(relx=0.5, rely=0.05, anchor='center')
        sc_sub = ctk.CTkLabel(sc_data,
                              text="Standardization scales the numerical features to have a mean of 0 and a standard deviation of 1.",
                              text_color="light grey")
        sc_sub.place(relx=0.5, rely=0.15, anchor='center')

        sc_label = ctk.CTkLabel(master=sc_data, text="Enter features to Standardize:", font=('bold', 15))
        sc_label.place(relx=0.5, rely=0.25, anchor='center')

        sc_entry = ctk.CTkEntry(master=sc_data, placeholder_text="Enter list of features separated with ','", width=250)
        sc_entry.place(relx=0.5, rely=0.4, anchor='center')

        sc_btn = ctk.CTkButton(master=sc_data, text="Standardize", font=('bold', 12), width=250,
                               command=standardize_features)
        sc_btn.place(relx=0.3, rely=0.5)

        save_sc_btn = ctk.CTkButton(master=sc_data, text="Save Current Dataset", font=('bold', 13), width=250,
                                    fg_color='green', command=export_dataset)
        save_sc_btn.place(relx=0.5, rely=0.8, anchor='center')
...
import_btn = ctk.CTkButton(master=dashboard_frame, font=('bold', 18), text='Import DataSet', compound='left' ,text_color='#e06d14',
                           fg_color='transparent', border_color='#e06d14', border_width=4, width=200, height=50, image=image_1, command=lambda: indicate(import_indicate, import_page))
...
vise_btn = ctk.CTkButton(master=dashboard_frame, font=('bold', 16), text='Data Visualisation', text_color='#e06d14',
                         fg_color='transparent', border_color='#e06d14', border_width=4, image=image_2, compound='left',width=200, height=50,
                         command=lambda: indicate(vise_indicate, vise_page))
...
process_btn = ctk.CTkButton(master=dashboard_frame, font=('bold', 18), text='Data Processing', text_color='#e06d14',
                            fg_color='transparent', border_color='#e06d14', border_width=4, image=image_3, compound='left', width=200, height=50,
                            command=lambda: indicate(process_indicate, process_page))
...
ml_btn = ctk.CTkButton(master=dashboard_frame, font=('bold', 18), text='Machine Learning', text_color='#e06d14',
                       fg_color='transparent', border_color='#e06d14', border_width=4, image=image_4, compound='left', width=200, height=50,
                       command=lambda: indicate(ml_indicate, ml_page))
...
tut_btn = ctk.CTkButton(master=dashboard_frame, font=('bold', 18), text='  Watch Tutorial', text_color='#e06d14',
                        fg_color='transparent', border_color='#e06d14', border_width=4,image=image_5, compound='left', width=200, height=50,
                        command=lambda: indicate(tut_indicate, tut_page))
...
doc_btn = ctk.CTkButton(master=dashboard_frame, font=('bold', 18), text='Documentation', text_color='#e06d14',
                        fg_color='transparent', border_color='#e06d14', border_width=4, image=image_6, compound='left',width=200, height=50,
                        command=lambda: indicate(doc_indicate, doc_page))
